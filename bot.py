...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if has_start_param:
        welcome_message = (
                "🎯 **آزمون آزمایشی قلمچی — رایگان!** 🎯\n\n"
                
                "🔥 **قلمچی امروز رو رایگان شرکت کن**\n"
                "✅ آزمون‌ قلمچی\n"
                "✅ شرایط واقعی سر جلسه\n"
                "✅ تراز + درصد + رتبه\n\n"
                ...
                "🚀 **برنامه شخصی‌سازی شده بعد از آزمون می‌گیری!**\n"
                "👇 از منوی زیر آزمون خودت رو شروع کن:"
        )
...
async def handle_field_selection(update: Update, context: ContextTypes.DEFAULT_TYPE, field: str):
    """پردازش انتخاب رشته تحصیلی - نسخه ساده"""
    persian_fields = {
        'experimental': 'تجربی',
        'math': 'ریاضی',
        'humanities': 'انسانی'
    ...
    simple_message = (
        ...
        "**مرحله ۳/۳: شرایط و امتحانات مدرسه**\n\n"
        "📝 لطفاً **شرایط خاص و امتحانات مدرسه** خود را وارد کنید:\n\n"
        "**می‌توانید موارد زیر را مشخص کنید:**\n"
        "• امتحانات مدرسه تا آزمون بعدی قلمچی\n"
        "• شرایط خاص (پشت کنکور، شروع از صفر و...)\n"
        "• پایه‌هایی که می‌خواهید بخوانید\n"
        "• محدودیت‌های زمانی\n"
        "• هر توضیح دیگری که لازم است\n\n"
        "**مثال‌هایی از پیام‌های قابل ارسال:**\n"
        "۱) پشت کنکور هستم و از صفر شروع می‌کنم. می‌خواهم همزمان پایه دهم و دوازدهم بخوانم.\n\n"
        "۲) امتحانات:\n"
        "ریاضی ۲۰ دی - فصل ۳ و ۴\n"
        "فیزیک ۲۵ دی - فصل ۲\n"
        "شیمی ۲۸ دی - فصل ۵\n"
        "در عین حال هفته‌ای ۳ روز کلاس زبان دارم.\n\n"
        "۳) دانش‌آموز دوازدهم تجربی، فقط می‌خواهم روی نیمسال دوم تمرکز کنم و امتحان نهایی دارم.\n\n"
...
async def handle_study_plan_text(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    """پردازش متن امتحانات وارد شده توسط کاربر - ثبت خودکار"""
    ...
    # ذخیره متن کاربر
    context.user_data['study_plan']['exams_info'] = text
    
    # ثبت خودکار و ارسال به ادمین
    try:
        ...
        logger.error(f"Error in study plan auto-submit: {e}")
        await update.message.reply_text(
